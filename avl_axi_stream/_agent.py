...
class Agent(avl.Agent):
    def __init__(self, name: str, parent: avl.Component) -> None:
        ...
        # Create sequencer and driver if transmitter is enabled
        if self.cfg.has_transmitter:
            self.tsqr = avl.Sequencer("tsqr", self)
            self.tseq = TransSequence("tseq",self.tsqr)
            self.tdrv = TransDriver("tdrv", self)
            self.tsqr.seq_item_export.connect(self.tdrv.seq_item_port)

        if self.cfg.has_receiver:
            self.rdrv = RecDriver("rdrv", self)
    ...
    async def run_phase(self) -> None:
        ...
        It initializes the agent and starts the transmitter and receiver if they are active.
        """
